...
Files/*.md に ## カテゴリ セクションと !include ディレクティブを挿入する。
...
def inject_into_files_md(files_md_path, groups, modules_dir, modules_rel, group_data):
    ...
    if "\n## カテゴリ\n" in content:
        print("  [skip] {}: ## カテゴリ section already exists".format(files_md_path.name))
        return
...
def inject_children_into_parent_groups(docs_dir, hierarchy):
    ...
    親グループ MD に ## カテゴリ セクションと !include ディレクティブを追記する。
    ...
    for parent_id in sorted(hierarchy.keys()):
        ...
        for modules_dir in sorted(docs_dir.rglob("Modules")):
            ...
            if "\n## カテゴリ\n" in content:
                print("  [skip] {}: ## カテゴリ section already exists".format(parent_md.name))
                continue
