...
description: |
  C のヘッダーやソースに Doxygen コメントを追加、修正するときや、リファクタリング後に
  コメントの対象を監査するときに使うスキルです。docs の原典と app/calc/prod の実例をもとに、
  記載場所、タグの使い分け、避ける書き方、確認項目をまとめます。
---
...
- 前置 Doxygen コメントは、直後の宣言または定義、もしくは `@file` や `@defgroup` などで明示した対象に対応させます
- 宣言や定義を削除、移動、別形式へ置換した場合は、`git diff` で前置コメントと後置コメントも確認します
- API 契約として必要な情報は新しい対象へ移し、既存説明と重複する情報は削除します
- ソース上の見出しや実装メモだけを残す場合は、通常コメントへ変更します
- マクロごとに後置コメントがある場合、マクロ群の見出しを前置 Doxygen コメントにしません
- Doxygen は対象を失ったコメントを別の宣言へ警告なしで結び付ける場合があるため、生成結果も確認します

判断基準と例は `docs/cheatsheet.md` の「Doxygen コメントの対象を明確にする」に従います。
...
1. 対象がヘッダーかソースかを確認し、`git diff` で削除または移動した宣言のコメントも確認する
...
- 宣言や定義を削除した後に、その前置 Doxygen コメントだけを残す
- 後置コメントを持つマクロ群の見出しを、先頭マクロの前置 Doxygen コメントとして記載する
...
- コメントがですます調で統一されているか
- コメントの記載場所が適切か
- 各 Doxygen コメントが、一つの宣言、定義、または明示的な Doxygen 対象に対応しているか
- リファクタリング前のコメントについて、移動、削除、通常コメント化の判断が完了しているか
- 生成文書で、コメントが意図しない宣言の説明へ結合されていないか
