...
Doxygen にスレッドセーフを表す専用コマンドはないため、`@par スレッド セーフティ` でユーザー定義の見出しとして代替します。`@par スレッド セーフティ` は `@warning` の直前に配置する慣例です。

スレッドセーフな関数の例を以下に示します。
...
`<caption>` はオプションで、画像のキャプションをダブルクォートで囲んで指定します。
