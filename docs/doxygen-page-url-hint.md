...
`makefile` は Doxygen 実行時に `GENERATE_TAGFILE` を `/tmp/doxyfw-tmp/{CATEGORY_ID}/run.XXXXXX/xml/doxyfw.tag` へ上書きする。
tag file は postprocess で参照するため、XML 中間ディレクトリは postprocess の後で削除する。
