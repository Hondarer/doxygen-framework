...
Doxygen の生成処理は制限が多く、記載した内容が正しくドキュメント化されない場合が多いため、本チート シートのひな型を活用して、高品質な Doxygen コメントを付与してください。

本チート シートに記載のない記法を用いる場合は、期待どおりに出力されるかどうか、生成結果を確認してください。
...
1. エディターで「インデント表示」機能 (例: VS Code の「render whitespace」) を有効にします。
2. コメント開始行のインデント位置を基準に、後続行がすべて揃っているか視認します。
3. 必要に応じて手動で先頭スペースを調整します。
...
宣言側に説明がある公開関数では、定義ごとにこのマーカーを置きます。  
先頭の公開関数に一度だけ置くのではなく、各関数に個別に配置します。
...
## Doxygen コメントの対象を明確にする

前置 Doxygen コメントは、直後の宣言または定義を文書化します。  
Doxygen は対象を失ったコメントを後続の別の宣言へ結び付ける場合があり、この誤結合では警告が出ないことがあります。

`/** ... */` 形式のコメントは、次のいずれかを満たす場所に記載してください。

- 文書化する宣言または定義の直前
- `@file`、`@defgroup`、`@page` など、対象を明示する Doxygen コマンドを含む場所
- `@ingroup` と `@{`、または `@}` によってグループの範囲を明示する場所

ソースを読む人だけに必要な見出しや実装メモには、通常コメント (`/* ... */`) を使用してください。

### リファクタリングで宣言や定義を変更する場合

宣言や定義を削除、移動、または別の形式へ置換する場合は、同じ変更で前置コメントと後置コメントを確認してください。

1. `git diff` で削除または移動した宣言の直前と同じ行にあるコメントを確認します。
2. API 契約として必要な情報は、新しい宣言の `@brief`、`@return`、タグなし本文などへ移行します。
3. 別の規範や API コメントに同じ情報がある場合は、重複するコメントを削除します。
4. ソース上の分類や実装メモとしてだけ必要な情報は、通常コメントへ変更します。
5. Doxygen の生成結果で、説明が意図しない宣言へ結び付いていないことを確認します。

Doxygen の警告がないことだけでは、コメントと宣言の対応が正しいとは判断できません。

### 後置コメントを持つマクロ群

各マクロが `/**< ... */` 形式の後置コメントを持つ場合、マクロ群の見出しを前置 Doxygen コメントとして記載しないでください。  
前置 Doxygen コメントはマクロ群ではなく先頭のマクロへ結び付き、先頭マクロの後置コメントと説明が結合されます。
...
/** @brief 解析エラーの詳細種別です。 */
#define SAMPLE_ERROR_NONE (0) /**< エラーなし。 */
```

生成文書にもマクロ群の説明が必要な場合は、ファイル コメント内の `@section` または `@defgroup` で対象を明示してください。  
ソース上の見出しだけが必要な場合は、通常コメントを使用します。
...
| 機能や型を提供します。 | ～を提供します。 |
| 概念や値を表します。 | ～を表します。 |
| インターフェイスを実装します。 | ～を実装します。 |
| 定数や種別を定義します。 | ～を定義します。 |
| メソッドや API を公開します。 | ～を公開します。 |
| 機能を補助します。 | ～をサポートします。 |
...
字下げが多いとプレーン テキストのコード ブロックとして解釈されるため、表を記述する際の列セパレータ `|` の字下げは行わないようにします。
...
表の見出しについて、見出し定義の `-` の個数 (バイト数) に切り捨てられる不具合があるため、見出しが漢字 (マルチバイト文字) の場合は `-` の個数を調整し、正しく表示されるかどうかを確認する必要があります (Doxygen 1.8.14 で現象を確認)。
...
CRT 薄ラッパーなど、内部に共有状態を持たず、引数 (ポインターや `FILE*` など) への同時アクセス保証を呼び出し側に求める関数では、引数名を主語にして条件を述べます。
