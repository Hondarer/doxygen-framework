...
`@brief` は YAML の構文と衝突しにくい短い文にし、詳細は `@details` や本文へ分けてください。
...
1. エディタで「インデント表示」機能（例：VS Code の「render whitespace」）を有効にする
...
ファイルコメント (`@file`) はブロックの前後にセパレータを置きます。
...
スレッドセーフかどうかを明記する場合は、`@par スレッド セーフ` を使用します。
Doxygen にスレッドセーフを表す専用コマンドはないため、`@par` でユーザー定義の見出しとして表現します。
...
Windows Web Services「スレッド セーフ」、Direct3D 11 のマルチ スレッド概要などの記述。
...
`@par` タグの前には空行を入れてください。空行がないと直前の `@details` や本文が `@par` の本文として扱われます。
...
CRT 薄ラッパーや内部に共有状態を持たないが、引数 (ポインタや `FILE*` など) に対する
