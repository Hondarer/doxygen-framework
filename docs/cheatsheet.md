...
1. `git diff` で削除または移動した宣言の直前と同じ行にあるコメントを確認する
2. API 契約として必要な情報は、新しい宣言の `@brief`、`@return`、タグなし本文などへ移す
3. 別の規範や API コメントに同じ情報がある場合は、重複するコメントを削除する
4. ソース上の分類や実装メモとしてだけ必要な情報は、通常コメントへ変更する
5. Doxygen の生成結果で、説明が意図しない宣言へ結び付いていないことを確認する
