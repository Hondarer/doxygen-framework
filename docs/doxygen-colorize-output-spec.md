# doxygen-colorize-output.sh 仕様

設計判断の背景は、[Doxygen 着色出力の調査結果](doxygen-colored-output-research.md) を参照してください。

## 概要

`doxygen-colorize-output.sh` は、Doxygen の出力メッセージに ANSI カラー コードを適用し、エラーと警告を視覚的に区別しやすくするフィルター スクリプトです。

## ファイル パス

```text
bin/doxygen-colorize-output.sh
```

## 実行権限

実行可能 (chmod +x)

## 機能

標準入力から受け取った各行を解析し、エラーまたは警告メッセージを検出して着色します。

### 着色ルール

| メッセージ タイプ | 検出パターン | ANSI カラー コード | 表示色 |
|------------------|--------------|-------------------|--------|
| エラー | ` error: ` | `\033[0;31m` | 🔴 赤 |
| 警告 | ` warning: ` | `\033[0;33m` | 🟡 黄 |
| その他 | (該当なし) | (なし) | 既定 |

## 実装詳細

### ANSI カラー コード定義

```bash
RED='\033[0;31m'
YELLOW='\033[0;33m'
RESET='\033[0m'
```

### 処理フロー

```plantuml
@startuml 処理フロー
caption 処理フロー

start
while (標準入力に行がある?) is (yes)
  :行を読み取り;
  if (' error: ' を含む?) then (yes)
    :赤色で出力;
  elseif (' warning: ' を含む?) then (yes)
    :黄色で出力;
  else (no)
    :そのまま出力;
  endif
endwhile (no)
stop

@enduml
```

### マッチング条件

#### エラーの検出

```bash
if [[ "$line" == *" error: "* ]]; then
```

行内に ` error: ` (前後にスペースを含む) が含まれる場合にマッチします。

#### 警告の検出

```bash
elif [[ "$line" == *" warning: "* ]]; then
```

行内に ` warning: ` (前後にスペースを含む) が含まれる場合にマッチします。

### 誤検知の防止

スペースを含むパターン マッチングにより、以下のような誤検知を防止します。

- 変数名: `show_error_flag`, `warning_count`
- 関数名: `print_error()`, `handle_warning()`
- コメント内の単語: `// This is an error example`

## 使用方法

### makefile からの呼び出し

```bash
doxygen Doxyfile 2>&1 | $(MAKEFILE_DIR)/bin/doxygen-colorize-output.sh
```

- `2>&1`: stderr を stdout にリダイレクトして結合
- `|`: パイプでフィルター スクリプトに渡す

### 終了コードの保持

```bash
doxygen Doxyfile 2>&1 | $(MAKEFILE_DIR)/bin/doxygen-colorize-output.sh;
DOXYGEN_EXIT=${PIPESTATUS[0]};
exit $DOXYGEN_EXIT;
```

`PIPESTATUS[0]` を使用して、パイプの最初のコマンド (doxygen) の終了コードを取得し、ビルドの成否を正しく判定します。

## 出力例

### 着色前

```text
/path/to/file.c:42: warning: undocumented parameter 'foo'
/path/to/file.c:100: error: invalid syntax
Normal output line
```

### 着色後

(ターミナルでは以下のように表示されます)

<span style="color: #ffaa00">/path/to/file.c:42: warning: undocumented parameter 'foo'</span>
<span style="color: #ff0000">/path/to/file.c:100: error: invalid syntax</span>
Normal output line

## 制限事項

### ANSI カラー コード非対応環境

Windows コマンド プロンプトなど、ANSI カラー コードに対応していないターミナルでは、エスケープ シーケンスがそのまま表示される可能性があります。

### ログ ファイルへの出力

ANSI カラー コードを含む出力をファイルにリダイレクトすると、エスケープ シーケンスがそのまま記録されます。

ログ ファイルに保存する場合は、以下のいずれかの方法を推奨します。

- フィルターを適用しない Doxygen の直接実行
- `WARN_LOGFILE` 設定による Doxygen からの直接ログ ファイル出力

## テスト方法

以下のコマンドでスクリプト単体のテストが可能です。

```bash
cat <<'EOF' | bin/doxygen-colorize-output.sh
Normal output line
/path/to/file.c:42: warning: undocumented parameter 'foo'
Another normal line
/path/to/file.c:100: error: invalid syntax
More normal output
EOF
```

期待される結果: warning 行が黄色、error 行が赤色で表示されます。

## 関連ファイル

- `makefile`: 本スクリプトを doxygen 実行時に適用
- `Doxyfile`: Doxygen の警告書式設定 (`WARN_FORMAT`)
- `docs/doxygen-colored-output-research.md`: 調査結果と背景情報
