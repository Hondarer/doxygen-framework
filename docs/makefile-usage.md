...
`app/{CATEGORY}/makefile` の doxy ターゲットは `prod/Doxyfile.part*` を列挙し、`Doxyfile.part`、`Doxyfile.part.<SUBCATEGORY>` の各ファイルに対して doxyfw を 1 回ずつ呼び出します。警告ファイルは各 SUBCATEGORY ごとに独立します (`doxy.warn`、`doxy_<SUBCATEGORY>.warn`)。スキップ判定 (`make_doxy.log`) は app 全体で共通です。
