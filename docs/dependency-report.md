...
対象範囲内の他関数を呼び出さない関数を依存度の低い関数として扱い、その関数を呼び出す側へ向かって依存 level を上げます。

このレポートは、変更リスクを完全に判定するものではありません。  
Doxygen が XML に出力した `references`、`referencedby`、`define` の `initializer` 内の参照に基づくため、関数ポインター、XML に参照情報が出ないマクロ経由、条件付きコンパイルで除外された呼び出し、対象範囲外ライブラリへの呼び出しは分析対象に含まれない場合があります。
...
また、ワークスペースの `.vscode/git_link.yaml` に `gitLinkHostProvider` が指定されている場合は、Source リンクの Git URL 生成にも同じ host/provider/webhost 読み替えを適用します。
...
`leaf-static`、`include-static-leaf`、`include-internal-static-leaf`、`leaf-global` は、対象範囲内の呼び出し元数を基準値に加えた値になります。  
`leaf-static` は `0` 番台、`include-static-leaf`、`include-internal-static-leaf`、`leaf-global` は `1000` 番台を使います。  
...
`include` と `include_internal` の関数は、ヘッダーに置かれた inline 関数や内部補助 API として使われることがあります。  
呼び出し種別の判定では `include` と `include_internal` を `libsrc` 相当領域として扱います。  
このため、`include_internal` 間の別ファイル呼び出しや `libsrc` / `src` から `include` / `include_internal` への呼び出しは `libsrc-file-caller` に分類します。
...
`libsrc` を含むパスは `libsrc`、`src` を含むパスは `src`、`include_internal` を含むパスは `include_internal`、`include` を含むパスは `include`、それ以外は `other` です。  
このカテゴリは `sourceArea` として表示に使います。  
呼び出し種別の判定では `include` と `include_internal` を `libsrc` 相当領域として扱います。
...
関数ポインター、マクロ展開後の呼び出し、プリプロセッサ条件で変わる呼び出しは、Doxygen XML に出力されない場合があります。

対象範囲外の関数は level 算出に含めません。  
たとえば標準ライブラリや別カテゴリの関数を呼び出していても、同じ XML 出力に関数として含まれていなければ呼び出し先数には含まれません。
