...
""",
            ...
            self.assertIn("runOverviewLayout({ movingNodeIds, anchorCenters, immediate, onComplete });", index_html)
            self.assertIn("function renderOverviewGraph(opts)", index_html)
            self.assertIn("const renderOpts = { immediate, onComplete: finishImmediateRefresh };", index_html)
            ...
            self.assertIn("onBeforeAnimation: clearOverviewLayoutPendingLabel", index_html)
